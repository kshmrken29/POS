...
<body>
  ...
  <div class="container">
    ...
    if (mysqli_num_rows($menu_items) > 0) {
        ...
        while($item = mysqli_fetch_assoc($menu_items)) {
            ...
            $servings_sold[] = $item['servings_sold'];
            $servings_available[] = $item['number_of_servings'] - $item['servings_sold'];
        }
    }
    ?>
                                ...
                                if (mysqli_num_rows($menu_items) > 0) {
                                    ...
                                    while($item = mysqli_fetch_assoc($menu_items)) {
                                        ...
                                        echo '<tr>
                                                ...
                                                <td class="' . $performance_class . '">' . 
                                                    round($performance) . '%</td>
                                              </tr>';
                                    }
                                } else {
    ...
    <div class="row mt-4">
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h4>Servings Sold vs Available</h4>
                </div>
                <div class="card-body">
  ...
  <script>
    ...
    const expectedSales = <?php echo json_encode($expected_sales); ?>;
    const servingsSold = <?php echo json_encode($servings_sold); ?>;
    const servingsAvailable = <?php echo json_encode($servings_available); ?>;
    ...
    // Servings chart
    const servingsCtx = document.getElementById('servingsChart').getContext('2d');
    const servingsChart = new Chart(servingsCtx, {
        type: 'bar',
        data: {
            labels: menuNames,
            datasets: [
